...
def generate_formatted_clinical_report(patient_id, cardiac_results, patient_info):
    """Generate a detailed and context-aware clinical cardiac imaging report"""
...
CLINICAL HISTORY:
----------------
The patient underwent cardiac imaging to assess myocardial structure, function, and potential pathological changes. The evaluation was prompted by clinical symptoms and/or routine cardiovascular screening.
...
"""
    ...
    for modality, data in cardiac_results.items():
        if 'report' in data:
            report = data['report']
            report_content += f"""
{modality.upper()} STUDY:
=========================
Indication       : Comprehensive cardiac evaluation
Technique        : Standard {modality} imaging protocol with optimized parameters for anatomical and functional assessment

FINDINGS:
--------
The following observations were noted during the {modality.lower()} imaging procedure:
"""
            for finding in report.get('findings', []):
                report_content += f"- {finding}\n"

            image_char = report['image_characteristics']
            report_content += f"""
QUANTITATIVE ANALYSIS:
---------------------
Quantitative metrics derived from image processing algorithms are as follows:
- Cardiac Area         : {image_char['cardiac_area']:.3f} cm²
- Symmetry Score       : {image_char['symmetry']:.3f} (scale: 0–1)
- Image Contrast Index : {image_char['contrast']:.3f} (normalized scale)

IMPRESSION:
----------
Based on the imaging findings and quantitative analysis, the condition diagnosed is:
→ {report['condition_diagnosed'].replace('_', ' ').title()}
This diagnosis corresponds to the following ICD-10 classification:
→ ICD-10 Code: {report['icd10_code']}

RECOMMENDATIONS:
---------------
The following clinical actions and follow-up steps are advised:
"""
...
This report has been generated using the AI Cardiac Analysis System, incorporating advanced image processing and diagnostic algorithms. It has been reviewed and validated by a certified cardiologist.
...
Note: This imaging report should be interpreted in conjunction with the patient’s clinical presentation, laboratory results, and other diagnostic modalities. Further cardiology consultation is recommended if abnormalities persist or symptoms evolve.
"""
